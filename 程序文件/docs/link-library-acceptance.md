...
用户指出文件名像乱码后，已将正式目录统一为“平台 - 标题 [短编号]”，主视频/音频带标题，截图/图片文字带时间和顺序，分 P、字幕语言及机器/平台来源分别命名。资料 ID、资产 ID 与采集工作路径保持独立，重试会恢复原工作路径并复用已保存文件。资料根文件为 `阅读.html`、`阅读.pdf`、`资料正文.md`、`原始正文.md`，管理清单仍为 `content.json`。

旧库整理与下载共用 worker 锁；先校验和复制，发布新目录、重建 PDF 后再把原目录移到 `.命名备份/`。记录每项新旧路径，发布中断可以恢复，PDF 失败会保留原资料。未被清单索引的旧旁路文件和历史版本仍保留在备份原目录。

正式 B 站样本已实际迁移到 `资料库/items/B站 - 2019 bilibili live star年度总选盛典宣传片（全片） [1badb59a]/`。51 项资产保留，除重新生成的 PDF 外，所有资产文件与原文件校验值一致；队列文件字节未变。Edge 实际打开中文路径阅读页，校验所有本地链接/图片，并播放原视频成功，远程请求为 0；重复整理为无操作。证据：`logs/readable-names-acceptance/report.json` 和同目录截图。前文旧 `items/<ID>/` 路径属于历史验证位置，当前路径以上述命名为准。

最终全量测试 **174/174 通过，0 失败、0 跳过**，构建与类型检查通过；日志为 `logs/readable-names-tests.txt`。回归覆盖中文/空格/括号、同名冲突、长名称和 Windows 保留名、增量检查点顺序、格式变化、旧路径兼容、迁移恢复、正文引用、导出脱敏、重试复用及原 Windows 窗口流程。当前运行的窗口可点击“全部”重新加载新地址。
