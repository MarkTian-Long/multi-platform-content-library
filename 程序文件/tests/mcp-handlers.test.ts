...
test("handler preserves browser startup failure and strips sensitive browser logs", async () => {
  const result = await captureWechatArticle({ url: "https://mp.weixin.qq.com/s/a" }, { capture: async () => { throw new Error("browserType.launchPersistentContext: spawn EPERM\nBrowser logs:\n--token=private-token https://mp.weixin.qq.com/s/a?key=secret"); } });
  assert.equal(result.status, "failed");
  assert.match(result.reason ?? "", /EPERM/);
  assert.match(result.reason ?? "", /权限|拒绝/);
  assert.doesNotMatch(result.reason ?? "", /private-token|key=secret/);
});
