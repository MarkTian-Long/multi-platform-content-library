...
test("CLI emits a local directory for a successful capture", async () => {
  ...
  assert.deepEqual(result, {
    ...
    directory: "D:/reader/文章库/2026-09-03_测试文章_b758e4d8", imageSummary: { total: 0, saved: 0, failed: 0 }, message: "文章已保存到本地"
  });
});
...
test("CLI rejects commands without a WeChat article link", async () => {
  ...
  assert.equal(result.message, "用法：capture <微信公众号文章链接>");
});